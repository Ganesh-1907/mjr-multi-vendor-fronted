...
@Component({
  ...
  styles: [`
    ...
    .stat-value { font-size: 32px; font-weight: 700; color: #212121; }
    .stat-label { font-size: 14px; color: #757575; }
    .dashboard-grid { display: grid; grid-template-columns: 2fr 1fr; gap: 24px; }
    .recent-orders table { width: 100%; border-collapse: collapse; }
    .recent-orders th, .recent-orders td { padding: 12px; text-align: left; border-bottom: 1px solid #eee; }
    .recent-orders th { font-weight: 600; color: #666; }
    .recent-orders a { color: #1a237e; text-decoration: none; }
    ...
    .quick-links button { justify-content: flex-start; }
...
export class CustomerDashboardComponent {
  ...
  orders = this.dataService.getOrders().slice(0, 5);
  pendingOrders = this.orders.filter(o => o.status === 'pending' || o.status === 'processing' || o.status === 'shipped').length;
  wishlistCount = 3;
}
